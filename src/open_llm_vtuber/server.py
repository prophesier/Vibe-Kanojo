...
class WebSocketServer:
    ...
    # cache/ entries that must SURVIVE the atexit wipe. Discord expression
    # faces can only be regenerated by /refresh-faces with a connected
    # frontend — the blanket wipe used to orphan them whenever the desktop
    # client wasn't running at the next restart.
    _CACHE_KEEP = frozenset({"discord_faces"})

    @classmethod
    def clean_cache(cls):
        """Clean the cache directory, preserving the keep-list entries."""
        cache_dir = "cache"
        if not os.path.exists(cache_dir):
            os.makedirs(cache_dir)
            return
        for name in os.listdir(cache_dir):
            if name in cls._CACHE_KEEP:
                continue
            ...
            try:
                ...
                logger.warning(f"clean_cache: could not remove {path}: {e}")
