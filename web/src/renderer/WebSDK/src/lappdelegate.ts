...
// Pet-mode gaze tuning (used in onMouseMoved). The cursor stays close to the
// model while dragging, so the relative offset is small — amplify it. The gaze
// should pivot around the head, which sits above the model-matrix centre.
const PET_GAZE_GAIN = 5.0; // amplify head turn
const PET_HEAD_OFFSET_Y = 0.6; // head centre above model centre, model-local units
...
function onMouseMoved(e: MouseEvent): void {
  ...
  // Pet mode: the canvas is a full-screen overlay and the model is moved to a
  // screen corner via its model matrix. The default transform makes the gaze
  // track the cursor's absolute screen position, so the model always looks
  // toward the corner it sits in. Instead, compute the cursor relative to the
  // model's own on-screen box (its matrix centre + scale) so the gaze tracks
  // where on the model the cursor is.
  if ((window as any).__isPetMode) {
    const view = LAppDelegate.getInstance().getView();
    const model = LAppLive2DManager.getInstance().getModel(0) as any;
    if (view && model && model._modelMatrix) {
      const scale: number = canvas!.width / rect.width;
      const cursorX: number = view._deviceToScreen.transformX(posX * scale);
      const cursorY: number = view._deviceToScreen.transformY(posY * scale);
      const m = model._modelMatrix.getArray();
      // Tunable at runtime from the Live2D settings sliders (window globals),
      // falling back to the constants above. X pivots at the model centre,
      // Y at the head (above centre); amplified so the turn is visible.
      const gain: number = (window as any).__petGazeGain ?? PET_GAZE_GAIN;
      const headOffsetY: number =
        (window as any).__petHeadOffsetY ?? PET_HEAD_OFFSET_Y;
      const relX: number = ((cursorX - m[12]) / (m[0] || 1)) * gain;
      const relY: number = ((cursorY - m[13]) / (m[5] || 1) - headOffsetY) * gain;
      LAppLive2DManager.getInstance().onDrag(relX, relY);
    }
    return;
  }
