...
export interface ModelInfo {
  ...
  /** Pet-mode gaze: head-turn amplification (default 5.0) */
  petGazeGain?: number;

  /** Pet-mode gaze: head pivot offset above model centre, model-local units (default 0.6) */
  petHeadOffsetY?: number;
}
  ...
  const setModelInfo = (info: ModelInfo | undefined) => {
    ...
    setModelInfoState({
      ...
      petGazeGain:
        "petGazeGain" in info
          ? info.petGazeGain
          : (modelInfo?.petGazeGain ?? 5.0),
      petHeadOffsetY:
        "petHeadOffsetY" in info
          ? info.petHeadOffsetY
          : (modelInfo?.petHeadOffsetY ?? 0.6),
    });
  };
