...
export const useLive2DModel = ({
  ...
  // Mirror the pet-mode gaze tuning into globals that the WebSDK
  // (lappdelegate.onMouseMoved) reads, so the settings sliders take effect live.
  useEffect(() => {
    (window as any).__petGazeGain = modelInfo?.petGazeGain ?? 5.0;
    (window as any).__petHeadOffsetY = modelInfo?.petHeadOffsetY ?? 0.6;
  }, [modelInfo?.petGazeGain, modelInfo?.petHeadOffsetY]);
